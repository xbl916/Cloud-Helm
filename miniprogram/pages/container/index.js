  ...
  async load() {
    ...
    try {
      ...
      const container = {
        ...
        networkReceive: format.rate(raw.network_rx_bps),
        networkTransmit: format.rate(raw.network_tx_bps),
        networkReceiveTotal: format.bytes(raw.network_rx_bytes),
        networkTransmitTotal: format.bytes(raw.network_tx_bytes),
        writableLayer: format.bytes(raw.writable_layer_bytes),
        rootfs: format.bytes(raw.rootfs_bytes),
        blockRead: format.rate(raw.block_read_bps),
        blockWrite: format.rate(raw.block_write_bps),
        blockReadTotal: format.bytes(raw.block_read_bytes),
        blockWriteTotal: format.bytes(raw.block_write_bytes),
        finishedAt: raw.finished_at && !raw.finished_at.startsWith("0001-") ? format.time(raw.finished_at) : "—",
